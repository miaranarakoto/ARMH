...
new Vue({
  ...
  methods: {
    selectScene (sceneId) {
      const between = Math.abs(this.scene - sceneId)

      if (sceneId < this.scene) this.direction = 'up'
      ...
      if (this.direction === 'up') {
        tl
          .to('#js-scene_nav_mark', 0.6, {
            ease: Power3.easeIn,
            width: (buttonSize + buttonMargin) * between + buttonSize + 'px',
            x: (buttonSize + buttonMargin) * sceneId + 'px'
          })
          .to('#js-scene_nav_mark', 0.6, {
            ease: Power3.easeOut,
            width: buttonSize
